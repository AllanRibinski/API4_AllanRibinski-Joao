...
app.delete('/tasks/:id/update', (req,res) =>{
    const id = req.params.id;
    const post_data = req.body;
    ...
    if(post_data.task == undefined || post_data)
    {
        res.json(functions.response('Atenção', 'Dadso inválidos', 0,null))
        return;
    }
    ...
    connection.query('UPDATE taks SET task=?, updated_at = NOW() WHERE id=?', [task,status,id],
        (err,rows)=>{
            if(!err)
            {
                res.json(functions.response('Sucesso', 'Task atualizada', 0, null));
            }
            else
            {
                res.json(functions.response('Sucesso', 'Task não encontrada', 0, null));
            }
        }
